...
description = "Angefrage befehle erlauben"
...
async def execute(message):
    ...
    if len(params) != 2:
        await message.channel.send("Fehler: Falsche Parameteranzahl\nBitte verwende die Syntax ``+enablecmd [username]#[0000] [command_name]``")
        return

    save_p0 = re.sub(r'[\\/*?:"<>|]', '', params[0])
    save_p1 = re.sub(r'[\\/*?:"<>|]', '', params[1])
    f_path = helpers.dir_path + "/server_proposed_actions/" + str(message.channel.original.guild.id) + "/" + save_p0 + "/" + save_p1 + ".py"
    print(f_path)
    ...
    f_dest_path = helpers.dir_path + "/server_actions/" + str(message.channel.original.guild.id) + "/" + save_p0 + "#" + save_p1 + ".py"
    shutil.move(f_path, f_dest_path)

    await message.channel.send("Server-Befehl erfolgreich aktiviert\nEr kann mit ``+disablecmd [username]#[0000] [command_name]`` wieder deaktiviert werden")
